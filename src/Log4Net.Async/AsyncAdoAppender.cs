    ...
    public class AsyncAdoAppender : AdoNetAppender
    {
        ...
        protected override void OnClose()
        {
            ...
            if (!hasFinished)
            {
                ...
                {
                    Level = Level.Error,
                    Message = "Unable to clear out the AsyncAdoAppender buffer in the allotted time, forcing a shutdown",
                    ...
                    LocationInfo = new LocationInfo(this.GetType().Name, "OnClose", "AsyncAdoAppender.cs", "75"),
        ...
        private void LogAppenderError(string logMessage, Exception exception)
        {
            base.Append(new LoggingEvent(new LoggingEventData
            {
                ...
                Domain = AppDomain.CurrentDomain.FriendlyName,
                ThreadName = Thread.CurrentThread.ManagedThreadId.ToString(),
                LocationInfo = new LocationInfo(this.GetType().Name, "LogAppenderError", "AsyncAdoAppender.cs", "152"),
                LoggerName = this.GetType().FullName,
                Properties = new PropertiesDictionary(),
            }));
        }
        ...
        private void LogBufferOverflowError()
        {
            base.Append(new LoggingEvent(new LoggingEventData
            {
                ...
                Domain = AppDomain.CurrentDomain.FriendlyName,
                ThreadName = Thread.CurrentThread.ManagedThreadId.ToString(),
                LocationInfo = new LocationInfo(this.GetType().Name, "LogBufferOverflowError", "AsyncAdoAppender.cs", "152"),
                LoggerName = this.GetType().FullName,
                Properties = new PropertiesDictionary(),
            }));
        }
